...
def Database():
    ...
    cursor.execute("CREATE TABLE IF NOT EXISTS `product` (product_id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, product_name TEXT, product_qty TEXT, product_price TEXT,producttotal integer)")
    cursor.execute("SELECT * FROM `admin` WHERE `username` = 'Navjeet' AND `password` = 'Shubham '")
    if cursor.fetchone() is None:
        cursor.execute("INSERT INTO `admin` (username, password) VALUES('Navjeet', 'Shubham')")
        conn.commit()

def Exit():
    result = tkMessageBox.askquestion('PRODUCT DETAILS', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        root.destroy()
        exit()
...
def ShowLoginForm():
    global loginform
    loginform = Toplevel()
    loginform.title("PRODUCT DETAILS/Account Login")
    width = 600
    height = 500
    screen_width = root.winfo_screenwidth()
    screen_height = root.winfo_screenheight()
...
def Home():
    global Home
    Home = Tk()
    Home.title("PRODUCT DETAILS/Home")
    width = 1024
    height = 520
    ...
    Title.pack(pady=10)
    lbl_display = Label(Title, text="PRODUCT DETAILS", font=('arial', 45))
    lbl_display.pack()
    menubar = Menu(Home)
...
def ShowAddNew():
    global addnewform
    addnewform = Toplevel()
    addnewform.title("PRODUCT DETAILS/Add new")
    width = 600
    height = 500
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def AddNewForm():
    ...
    lbl=Label(MidAddNew,text="Total",font=('arial',25),bd=10)
    lbl.grid(row=3 , sticky=W)
    productname = Entry(MidAddNew, textvariable=PRODUCT_NAME, font=('arial', 25), width=15)
    productname.grid(row=0, column=1)
    global productqty,productprice
    ...
    #producttotal=Entry(MidAddNew,textvariable=Product_total,font=('arial',25),width=15)
    #producttotal.grid(row=3,column=1)
...
def AddNew():
    Product_total=IntVar()
    product_total=int(productqty.get())* int(productprice.get())
    label=Label(MidAddNew,text=str(product_total),font=('arial',25))
    label.grid(row=3)
    Database()
    cursor.execute("INSERT INTO `product` (product_name, product_qty, product_price, producttotal) VALUES(?, ?, ?,?)", (str(PRODUCT_NAME.get()), int(PRODUCT_QTY.get()), int(PRODUCT_PRICE.get()),int(Product_total.get())))
    conn.commit()
    PRODUCT_NAME.set("")
    PRODUCT_PRICE.set("")
    PRODUCT_QTY.set("")
...
def Delete():
    if not tree.selection():
       print("ERROR")
        ...
        result = tkMessageBox.askquestion('PRODUCT DETAILS', 'Are you sure you want to delete this record?', icon="warning")
        if result == 'yes':
            curItem = tree.focus()
...
def ShowView():
    global viewform
    viewform = Toplevel()
    viewform.title("PRODUCT DETAILS/View Product")
    width = 600
    height = 400
    screen_width = Home.winfo_screenwidth()
    screen_height = Home.winfo_screenheight()
...
def Logout():
    result = tkMessageBox.askquestion('PRODUCT DETAILS', 'Are you sure you want to logout?', icon="warning")
    if result == 'yes': 
        admin_id = ""
        root.deiconify()
        Home.destroy()
